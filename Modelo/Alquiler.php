...
class Alquiler {
    ...
    public function RegistrarAlquiler(){
        ...
        $sql = "INSERT INTO alquiler (fecha,horaSalida,cliente,"
                . "estado,idBicicleta,idUsuario) VALUES (?,?,?,?,?,?)";
        $insertar = $ic->db->prepare($sql);
        $insertar->bindParam(1, $this->Fecha);
        $insertar->bindParam(2, $this->HoraSalida);
        $insertar->bindParam(3, $this->Cliente);
        $insertar->bindParam(4, $this->Estado);
        $insertar->bindParam(5, $this->Bicicleta);
        $insertar->bindParam(6, $this->Usuario);
        $insertar->execute();
    }
